...
常见问题 (FAQ)
启动失败 / 端口被占用怎么办？
修改启动端口：在"启动"页面中，修改端口号输入框的值（默认 20000），然后重新启动。如果端口被系统占用，会提示"端口被占用"，改为其他端口（如 20001、30000）即可。

如何更新模型？
在"模型"页面中重新下载模型（覆盖原有文件）
或者手动删除模型目录下的旧 GGUF 文件，重新下载
下载速度慢 / 下载失败？
检查网络连接和代理设置
点击"重试下载"按钮重新尝试
或前往官网 sal.bszx.site/download-page 使用备用高速下载链接
如何更新 SAL 本身？
启动时自动检查更新，如有新版本会在首页弹出更新卡片。点击"更新"跳转至官网下载新版 MSI 安装包覆盖安装。

模型加载后回答乱码 / 输出特殊符号？
部分模型可能输出 <|im_end|>、</s> 等特殊 token，SAL 已内置自动过滤机制。如仍有异常，可以尝试：

使用其他量化版本的模型（如 Q4_K_M 替换 Q8_0）
在"启动"页面调整上下文长度
如何查看日志？
托盘图标右键 -> 查看日志
或直接打开 %APPDATA%/SoulLogs/ 文件夹
支持 macOS / Linux 吗？
目前仅支持 Windows 10/11。Linux 和 macOS 版本暂未计划
